...
export const configureMonaco = (monaco: Monaco) => {
  ...
  // Error checking. The browser editor can't see the project's node_modules,
  // so every `import express from "express"` would be underlined as
  // "Cannot find module". Those error codes are ignored; real errors still show.
...
// Typed, so a wrong or outdated option name is a compile error instead of silently ignored
export const defaultEditorOptions: editor.IStandaloneEditorConstructionOptions = {
  ...
  occurrencesHighlight: "singleFile", // was `true`; Monaco now takes a mode
  
  // Suggestions
  suggestOnTriggerCharacters: true,
  acceptSuggestionOnEnter: "on",
  tabCompletion: "on",
  wordBasedSuggestions: "currentDocument", // was `true`; Monaco now takes a mode
  quickSuggestions: {
    other: true,
    comments: false,
    strings: false
  ...
  // Guides (renderIndentGuides/highlightActiveIndentGuide were renamed to `guides`)
  guides: {
    indentation: true,
  ...
  cursorSmoothCaretAnimation: "on", // was `true`; Monaco now takes "on" | "off" | "explicit"
  cursorStyle: "line",
  cursorWidth: 2,
